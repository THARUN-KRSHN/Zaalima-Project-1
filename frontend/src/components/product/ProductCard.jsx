...
export default function ProductCard({ product, variant = 'ecommerce' }) {
    ...
    const {
        id, // Ensure your product object has an 'id'
        title,
        brand,
        image,
        price,
    ...
    if (variant === 'overlay') {
        return (
            <div
                onClick={handleNavigation} // Added navigation trigger
                className="relative aspect-[4/5] w-full rounded-[1.25rem] sm:rounded-[2rem] overflow-hidden group shadow-[0_4px_12px_rgba(0,0,0,0.05)] select-none bg-[#1C1816] cursor-pointer"
            >
                <img
                    src={image}
                    alt={title}
    ...
    return (
        <div
            onClick={handleNavigation} // Added navigation trigger
            className="bg-[var(--bg-surface)] rounded-md border border-[var(--border-light)] hover:shadow-[var(--shadow-md)] transition-all duration-200 flex flex-col h-full group cursor-pointer select-none"
        >
